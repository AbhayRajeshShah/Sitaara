...
- [x] User can sign up with email + password, select role (Mom/Dad), create child profile
- [x] User can log in with email + password
- [x] Two seeded masterclasses exist, each with 4–5 real-duration videos
- [x] User can browse masterclasses and videos
- [x] User can play a video and see watch progress
- [x] Watch progress is reported to backend and persisted
- [x] User can like / unlike a video
- [x] Like state is persisted and retrieved
- [x] User can generate an invite code
- [x] User can redeem an invite code to link with their partner
- [x] After linking, both users can see each other's activity:
...
- [x] Partner activity is only visible after linking
- [x] Partner activity updates on refresh (consistency on refresh achieved)

### Edge Cases & Integrity (Must Have)

- [x] User cannot link to themselves (validation prevents self-linking)
- [x] Invite code cannot be redeemed twice (code is single-use)
- [x] Invite code cannot be redeemed by more than one partner (only redeemer becomes linked)
- [x] User who is already linked cannot silently re-link (error message or prevention)
- [x] Concurrent redemption of the same code is handled (only one succeeds)
- [x] Watch progress never goes backwards (furthest-watched wins)
- [x] Liking twice is idempotent (no duplicate like records)
- [x] User cannot see any other user's data (except linked partner on shared child)
- [x] Authorization checks prevent unauthorized access to partner data

### Data & Persistence (Must Have)

- [x] PostgreSQL database is used and hosted (not localhost)
- [x] Data persists across sessions
- [x] Content is seeded on initialization (2 masterclasses, 4–5 videos each)
- [x] User data, child data, video progress, likes, and partner relationships are stored

### Deliverables (Must Have)

- [x] Live, hosted demo is accessible and fully functional
- [x] Source code is provided and readable
